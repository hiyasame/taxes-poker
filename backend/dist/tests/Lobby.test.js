...
const Lobby_1 = require("../src/game/Lobby");
...
describe('Lobby and Security', () => {
    ...
    it('should fallback to Waiting state if players < 2', () => {
        ...
        lobby.startGame();
        expect(lobby.game.state).not.toBe(Game_1.GameState.Waiting);
        lobby.removePlayer('2');
        expect(lobby.game.state).toBe(Game_1.GameState.Waiting);
    ...
    it('should hide community cards that are not yet dealt', () => {
        ...
        expect(viewAfterFlop.state).toBe(Game_1.GameState.Flop);
